		...
		!std::is_convertible<T, std::shared_ptr<Json::Serializable>>::value>::type>::operator()(Json::ArrayType<T>& array) const
{
	if (json.pimpl) {
		try {
			for (const auto& i : json.pimpl->getArray()) {
				array.addElement(
					i.isEmpty() ? nullptr : std::make_shared<typename T::element_type>(i.convert<typename T::element_type>()));
			}
		} catch (Poco::Exception& e) {
			throw Error(e.message());
		}
...
#if (_SIZEOF_INT8_T + 0)
EOF
GetArrayElements_CPP std::int8_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_INT16_T + 0)
EOF
GetArrayElements_CPP std::int16_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_INT32_T + 0)
EOF
GetArrayElements_CPP std::int32_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif
...
#if (_SIZEOF_INT8_T + 0)
EOF
GetArrayElements_shared_ptr_CPP std::int8_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_INT16_T + 0)
EOF
GetArrayElements_shared_ptr_CPP std::int16_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_INT32_T + 0)
EOF
GetArrayElements_shared_ptr_CPP std::int32_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif
...
EOF
GetArrayElements_CPP std::uint8_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_UINT16_T + 0)
EOF
GetArrayElements_CPP std::uint16_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_UINT32_T + 0)
EOF
GetArrayElements_CPP std::uint32_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif
...
#if (_SIZEOF_UINT8_T + 0)
EOF
GetArrayElements_shared_ptr_CPP std::uint8_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_UINT16_T + 0)
EOF
GetArrayElements_shared_ptr_CPP std::uint16_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif

#if (_SIZEOF_UINT32_T + 0)
EOF
GetArrayElements_shared_ptr_CPP std::uint32_t >> ${FILENAME}
cat << EOF >> ${FILENAME}
#endif
