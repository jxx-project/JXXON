...
template<>
GetMapElements<std::string, Base::UnorderedMap>::GetMapElements(const Json& json) : json(json)
{
}
...
{
	populateMap<std::string, Base::UnorderedMap>(map, json.pimpl->value, [](const ::Json::Value::const_iterator& i){return i->asString();});
}
