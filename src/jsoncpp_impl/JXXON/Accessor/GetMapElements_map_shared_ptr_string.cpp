...
template<>
GetMapElements<std::shared_ptr<std::string>, std::map>::GetMapElements(const Json& json) : json(json)
{
}
...
{
	populateMap<std::shared_ptr<std::string>, std::map>(map, json.pimpl->value, [](const ::Json::Value::const_iterator& i){return i->asString();});
}
