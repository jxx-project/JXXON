...
template<>
GetMapElements<unsigned int>::GetMapElements(const Json& json) : json(json)
{
}

template<>
void GetMapElements<unsigned int>::operator()(Json::MapType<unsigned int>& map) const
{
	populateMap<unsigned int>(map, json.pimpl->value, [](const ::Json::Value::const_iterator& i){return i->asUInt();});
}
